...
public class MaxBinaryHeap<E> {
    ...
    public MaxBinaryHeap(int initCapacity, Comparator<? super E> comparator) {
        ...
        if(initCapacity<1){
            throw new IllegalStateException("初始化堆的容量小于1");
        }
    ...
    private void buildHeap(Object[] heap) {
        ...
        这里元素的索引是从0开始的，所以最后一个非叶子节点array.length/2 - 1，
        这是利用了完全二叉树的性质*/
    ...
    private void buildHeap(Object[] arr, int i, int length) {
        ...
        for (temp = arr[i]; (childIndex = 2 * i + 1) < length; i = childIndex) {
            //childIndex + 1 < length 说明该节点具有右子节点,并且如果如果右子节点的值大于左子节点，
    ...
    private void addNode(E e, int start) {
        ...
        /*如果start>0 寻找合适的位置:在某个插入的位置的新节点小于等于对应的父节点的值*/
        while (start>0){
            //判断父节点和新子节点的大小,如果父节点大于等于新子节点,那么符合小顶堆的要求,重构结束,该start就是子节点插入的位置
    ...
    public boolean remove(E e){
        ...
        //构建之后如果eIndex位置的元素就是x，说明没有调整堆结构，
        // 那么将该位置的元素看成新插入的元素，需要向上构建大顶堆
        if(heap[eIndex]==x){
